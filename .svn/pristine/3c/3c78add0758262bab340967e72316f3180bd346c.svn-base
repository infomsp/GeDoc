...
<% Html.Telerik().Grid<GeDoc.expExpedientes>()
        .Name("GridBandejaIniciadosPorMi")
        .DataKeys(keys =>
        {
            keys.Add(p => p.Id);
        })
        ...
        {
            dataBinding.Ajax()
                .Select("_SelectEditingIniciadosPorMi", "expExpediente");
        })
        .Columns(columns =>
        {
            ...
            .ClientEvents(events => events.OnCommand("onCommand").OnRowSelected("onRowSelected").OnDataBinding("onDataBinding").OnComplete("onComplete").OnLoad("onLoad"))
            .Footer(true)
            .Filterable()
            .Selectable()
