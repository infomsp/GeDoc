...
<% Html.Telerik().Grid<GeDoc.expExpedientes>()
        ...
        {
            dataBinding.Ajax()
                .Select("_SelectEditingSalida", "expExpediente");
        })
        .Columns(columns =>
        {
            ...
            columns.Bound(c => c.Fecha).Width("140px").Title("Fecha").Visible(true);
            ...
            columns.Bound(c => c.nombreMovimiento).Width("250px").Title("A donde envío").Visible(true).Visible(true).HtmlAttributes(new { style = "white-space: nowrap;" })
            .ClientTemplate("<label title='<#= nombreMovimiento #>' style='cursor: pointer;' style='white-space: nowrap;'><#= nombreMovimiento #> </label>");
