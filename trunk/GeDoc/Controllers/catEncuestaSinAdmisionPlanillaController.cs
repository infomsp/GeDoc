        ...
        public int? getResuelto(int plaid) {
            ...
            foreach (var result in results) {
                ...
                if (result.atendidoLocal == "NO" && result.programado == "NO") {
                    ...
                    if (resultados.Contains(result.pacId))
                    {
                        //la nada misma
                    }
                    else {
                        resultados.Add(result.pacId);
                    }
        ...
        public int verificarEspecialidad(int? paciente, int? especialidad, int? planilla)
        {
            ...
            var result = (
                            ...
                            where d.espId == especialidad &&
                                  a.pacId == paciente &&
                                  a.plaId == planilla //agregado para corregir error que no dejaba cargar misma espec en distintas planillas
                            select (d)
                         ).Count();
            if (result > 0)
            {
